...
export default function Header() {
  ...
  return (
    ...
    >
      <header
        ref={headerRef}
        className={`w-full max-w-6xl pointer-events-auto transition-[background-color,border-color,box-shadow,padding] duration-300 ease-out-expo rounded-[28px] ${
          scrolled || isOpen
        ...
        style={{
          boxShadow: scrolled || isOpen
            ? "0 1px 2px rgba(11,27,46,0.05), 0 8px 32px -8px rgba(11,27,46,0.10), inset 0 1px 0 rgba(255,255,255,0.9)"
            : "0 1px 2px rgba(11,27,46,0.03), inset 0 1px 0 rgba(255,255,255,0.7)",
        }}
      >
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          <nav
            ...
            className="hidden lg:flex items-center gap-0.5 relative px-1 py-1 rounded-full border border-navy-deep/[0.06] bg-white/70 backdrop-blur-md"
        ...
        >
          <div className="overflow-hidden">
            <div className="mt-2 pt-2 pb-1.5 space-y-1 rounded-[28px] border border-navy-deep/[0.08] bg-white/85 backdrop-blur-xl backdrop-saturate-150 p-2"
              style={{
                opacity: isOpen ? 1 : 0,
                transform: isOpen ? "translateY(0)" : "translateY(-8px)",
                transition: "opacity 0.3s ease, transform 0.3s cubic-bezier(0.32,0.72,0,1)",
                boxShadow: "0 8px 32px -8px rgba(11,27,46,0.12), inset 0 1px 0 rgba(255,255,255,0.9)",
              }}
            >
              {navItems.map((item) => (
                <a
